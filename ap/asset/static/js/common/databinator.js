...
class Databinator {
    constructor(element) {
        if (!element) {
            throw new Error('Missing required argument. new Detabinator needs an element reference\n必要な引数がありません。 新しいDetabinatorには要素参照が必要です');
        }

        this._insert = false;
        this._focusableEmenentsString = 'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex], [contenteditable]';
        this._focusableEmenents = Array.from(
            element.querySelectorAll(this._focusableEmenentsString)
        );
    }
    ...
    set insert(isInsert) {
        if (this._insert === isInsert) {
            return;
        }

        this._insert = isInsert;

        this._focusableEmenents.forEach((child) => {
            if (isInsert) {
